...
group by utm_source, utm_medium, utm_campaign, daily_spent, campaign_date 
union all
select utm_source, utm_medium, utm_campaign, sum(daily_spent) as total_cost, to_char(campaign_date, 'YYYY-MM-DD') as campaign_date
from ya_ads ya
where utm_medium <> 'organic'
...
from sessions s
inner join leads l on s.visitor_id = l.visitor_id and s.visit_date <= l.created_at
...
from tab3
inner join tab on tab3.utm_campaign = tab.utm_campaign and tab3.utm_medium = tab.utm_medium and tab3.utm_source = tab.utm_source
